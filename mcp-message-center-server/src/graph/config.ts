...
export function getGraphEnvConfig(publicBaseUrl: string): GraphEnvConfig {
  const tenantId =
  ...
  const redirectUri =
    process.env.GRAPH_REDIRECT_URI ?? `${publicBaseUrl.replace(/\/$/, '')}/auth/graph/callback`;
