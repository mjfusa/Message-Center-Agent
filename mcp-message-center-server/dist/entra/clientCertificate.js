...
export function normalizePrivateKeyFromSecretValue(secretValue) {
    const trimmed = secretValue.trim();
    // Support either raw PEM, or JSON like {"privateKey":"..."}.
    if (trimmed.startsWith('{')) {
        try {
            const parsed = JSON.parse(trimmed);
            const candidate = (typeof parsed.privateKey === 'string' && parsed.privateKey) ||
                (typeof parsed.privateKeyPem === 'string' && parsed.privateKeyPem);
            if (candidate)
                return normalizePrivateKeyFromSecretValue(candidate);
        }
        catch {
            // fall through
        }
...
export async function getPrivateKeyFromKeyVault() {
    if (cachedPrivateKey)
    ...
    cachedPrivateKeyPromise = (async () => {
        ...
        if (!secret.value) {
            throw new Error(`Key Vault secret '${secretName}' in '${vaultUrl}' returned no value. Ensure it is a secret (not a certificate object) and has a value.`);
        }
...
export async function getClientCertificateFromEnvOrKeyVault() {
